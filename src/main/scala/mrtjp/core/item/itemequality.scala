...
class ItemEquality {
  ...
  def matches(key1: ItemKey, key2: ItemKey): Boolean = {
    ...
    if (key1.item == key2.item) {
      ...
      if (matchMeta) {
        if (
          stack1.isItemStackDamageable && stack2.isItemStackDamageable && damageGroup > -1
        ) {
          val percentDamage1 =
            stack1.getItemDamage.toDouble / stack1.getMaxDamage * 100
          val percentDamage2 =
            stack2.getItemDamage.toDouble / stack2.getMaxDamage * 100
